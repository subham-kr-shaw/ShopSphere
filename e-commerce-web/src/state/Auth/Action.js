...
import { API_BASE_URL } from "../../config/apiConfig"
import {
    GET_USER_SUCCESS, GET_USER_REQUEST, LOGIN_SUCCESS, LOGIN_FAILURE,
    LOGIN_REQUEST, REGISTER_FAILURE, REGISTER_REQUEST, REGISTER_SUCCESS,
    GET_USER_FAILURE, LOGOUT, CLEAR_ERROR
} from "./ActionType";
import axios from "axios";
...
export const register = (userdata) => async (dispatch) => {
    ...
    try {
        ...
        if (data.token) {
            ...
            localStorage.setItem("role", data.role || "customer"); // ✅ store role
        }
        dispatch({ type: REGISTER_SUCCESS, payload: { token: data.token, role: data.role || "customer" } });
    } catch (error) {
...
export const login = (userdata) => async (dispatch) => {
    ...
    try {
        ...
        dispatch({ type: LOGIN_SUCCESS, payload: { token: data.token, role: data.role || "customer" } });
    } catch (error) {
